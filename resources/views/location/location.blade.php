                ...
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title col-md-2">
                            <a href="{{route('locationreg')}}" class="btn btn-block btn-success">ADD</a>
                        </h3>
                    </div>
                    <div class="box-body">
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            @foreach($loc as $lt)
                                <tr>
                                    ...
                                    <td>{{$lt->location}}</td>
                                    <td>
                                        <a href="{{route('editlocation', ['id' => $lt->id]) }}" class="btn btn-block btn-info fa fa-pencil" aria-hidden="true"></a>
                                        <form action="{{route('deletelocation',['id'=> $lt->id] )}}" method="POST">
                                        @csrf
